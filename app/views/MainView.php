...
class MainView {
    public function showMainApp($core, $users) {
        $site_url = $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'];
        $return = "";
        $return .= <<<HTML
        <!-- <a class="control" id="zoomout">Zoom Out</a>
        <a class="control" id="zoomin">Zoom In</a> -->
        <span id="welcome">Hello {$core->getUser()->first_name}!</span>
        <div id="main"></div>
        <link rel="stylesheet" href="css/main.css">
        <!-- This script passes php variables to javascript -->
        <script>const site_url = '{$site_url}'; const users = {$users}; const username = '{$core->getUser()->username}'</script>
